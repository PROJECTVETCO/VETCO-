...
export default function DashboardPage() {
  ...
  const API_BASE_URL =
    process.env.NEXT_PUBLIC_API_BASE_URL || "https://vetco.onrender.com";
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        if (!statsResponse.ok) {
          console.error(
            `❌ Stats Fetch Error (${statsResponse.status}):`,
        ...
        if (!activityResponse.ok) {
          console.error(
            `❌ Activity Fetch Error (${activityResponse.status}):`,
            await activityResponse.text()
          );
          throw new Error(
            `Activity Fetch Error: ${activityResponse.statusText}`
          );
        }
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {loading ? "Loading..." : totalAppointments}
                </div>
                ...
                <div className="space-y-4">
                  {recentActivity.map((activity) => (
                    <div key={activity.id} className="flex items-center gap-4">
                      <div
                        className={`h-8 w-8 rounded-full bg-${activity.color}-100`}
                      ></div>
                      <div>
        ...
        <div className="mt-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Meet our professional veterinary doctors
              </CardDescription>
            </CardHeader>
            <CardContent>
              <DoctorCarousel />
